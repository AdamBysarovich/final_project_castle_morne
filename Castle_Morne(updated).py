...
def generate_rooms(n_rooms: int = 9) -> Dict[int, Room]:
    ...
    # Build a grid for exits (do AFTER rooms exist)
    ...
    for r in rooms.values():
        r.description += " " + random.choice([
            "You walk in and it reeks of rot.",
            "Scratches across the walls tell of past struggles.",
            "A chill breeze whispers.",
            "You can feel the magic in this place, but can't tell what kind."
        ])
...
def write_lore_file(filename: str, rooms:dict[int, Room], nuggets: int =5):
    ...
    lines.append("\nROOM TEASERS\n")
    for r in rooms.values():
        lines.append(f"{r.name}: {textwrap.shorten(r.description, width=60)}\n")
...
def load_game(filename: str = "savegame.txt") -> Optional[Dict]:
    ...
    for ln in lines:
        ...
        if parts[0] == "player":
            _, name, role_key, hp_str, room_str = parts
...
def choose_role_prompt() -> Role:
    ...
    while True:
        choice =input("Enter role key (e.g., necromancer) or 'list' to see roles again. ").strip().lower()
        if choice == "list":
    ...
    if os.path.exists("savegame.txt"):
        if input("Load previous save? (y/n): ").strip().lower() == "y":
            ...
            if data and data["player"]:
                ...
                place_items_randomly(rooms, n_items=5)
